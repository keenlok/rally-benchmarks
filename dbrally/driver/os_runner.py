...
from dbrally import exceptions, track
from dbrally.utils import convert
from dbrally.client import RequestContextHolder
# Mapping from operation type to specific runner
from dbrally.utils.parse import parse_int_parameter, parse_string_parameter
...
def register_default_runners():
    ...
    register_runner(track.OperationType.VectorSearch, Query(), async_runner=True)
    register_runner(track.OperationType.BulkVectorDataSet, BulkVectorDataSet(), async_runner=True)
    ...
    register_runner(track.OperationType.CreatePointInTime, CreatePointInTime(), async_runner=True)
    register_runner(track.OperationType.DeletePointInTime, DeletePointInTime(), async_runner=True)
    register_runner(track.OperationType.ListAllPointInTime, ListAllPointInTime(), async_runner=True)
    ...
    register_runner(track.OperationType.DeletePipeline, Retry(DeletePipeline()), async_runner=True)
    ...
    register_runner(track.OperationType.CreateSearchPipeline, Retry(CreateSearchPipeline()), async_runner=True)
    register_runner(track.OperationType.DeleteMlModel, Retry(DeleteMlModel()), async_runner=True)
    register_runner(track.OperationType.RegisterMlModel, Retry(RegisterMlModel()), async_runner=True)
    register_runner(track.OperationType.DeployMlModel, Retry(DeployMlModel()), async_runner=True)
